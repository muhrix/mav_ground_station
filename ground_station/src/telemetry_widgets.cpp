...
void updateWidgetsCallback(const asctec_hl_comm::mav_imuConstPtr& imu) {
	// **** get GTK thread lock
	gdk_threads_enter();
	...
	// **** update altimeter -- height [mm]
	if (IS_GTK_ALTIMETER (data->alt)) {
		gtk_altimeter_set_alti(GTK_ALTIMETER (data->alt), imu->height/1000.0);
	}

	// **** update variometer -- differential height [mm/s]
	if (IS_GTK_VARIOMETER (data->vario))
		gtk_variometer_set_value(GTK_VARIOMETER (data->vario), imu->differential_height/1000.0);
...
void *startROS(void *user) {
	if (user != NULL) {
		...
		if (!n_param.getParam("altimeter_unit_is_feet",
			...
			data->altimeter_unit_is_feet = false;
		...
		sprintf(data->gauge2_name_f, "<big>%s</big>\n<span foreground=\"orange\"><i>(%s)</i></span>", gauge2_name.c_str(), gauge2_unit.c_str());
		...
		ROS_DEBUG(
				"\tGauge 1 initial step value: %d", data->gauge2_initial_step);
		...
		sprintf(data->name_bar_gauge1_f, "<big>%s</big>\n<span foreground=\"orange\"><i>(%s)</i></span>", name_bar_gauge1.c_str(), unit_bar_gauge1.c_str());
		...
		sprintf(data->name_bar_gauge2_f, "<big>%s</big>\n<span foreground=\"orange\"><i>(%s)</i></span>", name_bar_gauge2.c_str(), unit_bar_gauge2.c_str());
		...
		sprintf(data->name_bar_gauge3_f, "<big>%s</big>\n<span foreground=\"orange\"><i>(%s)</i></span>", name_bar_gauge3.c_str(), unit_bar_gauge3.c_str());
		...
		// -----------------------------------------------------------------
		// **** topics subscribing
		ROS_INFO("Subscribing to fake data topic");
		ros::Subscriber sub;
		sub = n.subscribe("fake_data", 1, updateWidgetsCallback);
...
int main(int argc, char **argv) {
	...
	g_object_set(GTK_GAUGE (data->gauge2), "grayscale-color",
			data->grayscale_color, "radial-color", data->radial_color,
			"start-value", data->gauge2_start_value, "end-value",
			data->gauge2_end_value, "initial-step", data->gauge2_initial_step,
			"sub-step", (gdouble) data->gauge2_sub_step, "drawing-step",
			data->gauge2_drawing_step, NULL);
	...
	g_object_set(GTK_TURN_COORDINATOR (data->tc), "grayscale-color",
			data->grayscale_color, "radial-color", data->radial_color, NULL);

	gtk_box_pack_start(GTK_BOX (hbox_up), GTK_WIDGET (data->gauge2), TRUE, TRUE,
			0);
	gtk_box_pack_start(GTK_BOX (hbox_up), GTK_WIDGET (data->alt), TRUE, TRUE,
			0);
	gtk_box_pack_start(GTK_BOX (hbox_up), GTK_WIDGET (data->arh), TRUE, TRUE,
			0);
	gtk_box_pack_start(GTK_BOX (hbox_up), GTK_WIDGET (data->bg), TRUE, TRUE, 0);

	gtk_box_pack_start(GTK_BOX (hbox_down), GTK_WIDGET (data->tc), TRUE, TRUE,
			0);
	gtk_box_pack_start(GTK_BOX (hbox_down), GTK_WIDGET (data->gauge1), TRUE,
			TRUE, 0);
	gtk_box_pack_start(GTK_BOX (hbox_down), GTK_WIDGET (data->comp), TRUE, TRUE,
			0);
	gtk_box_pack_start(GTK_BOX (hbox_down), GTK_WIDGET (data->vario), TRUE,
			TRUE, 0);
